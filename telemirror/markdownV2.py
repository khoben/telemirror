...
def escape_md(text: str) -> str:
    return re.sub(pattern=MARKDOWN_ESCAPE_PATTERN, repl=r"\\\1", string=text)
...
def parse(message: str) -> Tuple[str, List[TypeMessageEntity]]:
    """
    ...
    for token in expand_inline_and_html(parsed):
        ...
        if token.type in ('blockquote_close', 'blockquote_open'):
            ...
            entities.append(MessageEntityPre(offset=len(
                message), length=len(token.content), language=''))
            message += token.content
        elif token.type == 'code_inline':
            entities.append(MessageEntityCode(
                offset=len(message), length=len(token.content)))
            message += token.content
        elif token.type in ('em_close', 'em_open'):
            push(MessageEntityItalic)
        elif token.type == 'fence':
            entities.append(MessageEntityPre(offset=len(
                message), length=len(token.content), language=token.info))
